...
/// Indicates that the allocator is allowed to be used where we panic on OOM.
pub trait Abort: AllocRef {}

impl Abort for Global {}
impl Abort for System {}

pub fn handle_alloc_error_audited<T, A: Abort>(
    layout: NonZeroLayout,
    error: Result<T, A::Error>,
) -> T {
    match error {
        Ok(t) => t,
        Err(_) => handle_alloc_error(layout.into()),
    }
}
...
#[repr(transparent)]
pub(crate) struct AbortAlloc<Alloc>(pub Alloc);

impl<A: Abort> AbortAlloc<A> {
    pub fn new_audited(a: A) -> Self {
        AbortAlloc(a)
    }
}
